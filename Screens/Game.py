...
class Game:
    def __init__(self, i_screen: Surface , menu):
        ...
        self.m_speed = 5
        self.x: float = + 20
        self.y: float = Config.HEIGHT - 69
        self.gravity = 0.8
        self.velocity_y = 0.0
        self.jump_speed_gravity = -20
        self.fall_line = 100
        ...
        self.platforms = [self.m_platform_rect, self.m_platform_rect, self.m_platform_rect]
    ...
    def run(self):
        ...
        while self.m_is_game_running:
           ...
           #-----------------x-----------------
            ...
            #לעבור על זה-----------------------------------------------------------
            # קוד עדכון אנכי עם בדיקת פגיעה נכונה
            self.velocity_y += self.gravity
            next_y = self.y + self.velocity_y

            # בדיקה אם השחקן יפול על הפלטפורמה
            ...
            for each_rect in self.platforms:
                each_rect = pygame.Rect(random.randint(0, 140), random.randint(200, 400), self.platform_width, self.platform_height)

            for each_rect in self.platforms:
                if self.m_player.player_rect.colliderect(each_rect):
                    if self.x > 0:
                        self.m_player.player_rect.right = each_rect.left
                    elif self.x < 0:
                        self.m_player.player_rect.left = each_rect.right
    ...
    def display_pause_overlay(self):
        ...
        #         self.overlay = pygame.image.load("Menu_images/MenuBG.jpg")
        #         self.background_image_game = pygame.image.load("Game_images/Game_BG.jpg")
        #         self.background_image_game = pygame.transform.scale(self.background_image_game,
        ...
        #
        #         # Objects
        #         self.m_player: Player = Player(self.x, self.y, self.m_width, self.m_height, (60, 170, 220),
        #                                        self.m_border_radius)
        #         self.m_platform: Platforms = Platforms(random.randint(0, 140), random.randint(200, 400),
        #                                                self.platform_width, self.platform_height, (60, 170, 220),
        #                                                self.platform_border_radius)
        #         self.m_platform_rect = pygame.Rect(0, 0, self.platform_width, 16)
        #         self.first_platform: Platforms = Platforms(self.platform_x, self.platform_y, Config.WIDTH,
        #                                                    self.platform_height, (200, 130, 190),
        #                                                    self.platform_border_radius)
        #         self.platforms = [self.m_platform_rect, self.m_platform_rect, self.m_platform_rect]
        #
        #     def run(self):
        ...
        #
        #             for each_rect in self.platforms:
        #                 each_rect = pygame.Rect(random.randint(0, 140), random.randint(200, 400), self.platform_width,
        ...
        # לי
        # בקוד
        # for שאני רוצה להדפיס בupdate_rect אבל אני לא יודע איך להדפיס כל rect התחלתי  להתבלבל, ועוד משהו אני חושב שהfor לפני שלeac_rect גם לא נכון, כי יש לולאה שמאתחלת כל rect רנדומלי ואז אחרי זה זה רץ בfor, אבל זה התחיל להתבלבל לי